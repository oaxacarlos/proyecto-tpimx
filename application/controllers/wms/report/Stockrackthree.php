...
class Stockrackthree extends CI_Controller{
  ...
  function index(){
      ...
      if(!isset($_SESSION['menus_list_user'][$this->config->item('wms_report_folder').'stockrackthree'])){
          ...
          $this->load->view('wms/report/stockrackthree/v_index', $data);
      }
  }
  ...
  function get_report(){
      ...
      $status = "'1','2'";
      $result = $this->model_tsc_item_sn->get_item_sn2_by_rack_by_status($status);

      $data["var_report"] = assign_data($result);
      $data["location"]   = $location;

      $this->load->view('wms/report/stockrackthree/v_report', $data);
  }
  //---
