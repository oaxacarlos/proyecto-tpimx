...
class Whsreceipt extends CI_Controller{
  ...
  function send_notif(){
      ...
      if(!is_null($data)){
          if(count($data) > 0){ // if there is data not process yet
              ...
              if($result) echo "Notification has beed sent...";
          }
          else{
            echo "No Data Yet.. All Data has been proceed..";
          }
      }
      else{ echo "No Data Yet.. All Data has been proceed.."; }
  }
  //---
  ...
  function send_email($data){
      ...
      // get send to
      $this->model_config->name = "email_whrcpt_nav_no_process_to";
      $send_to = $this->model_config->get_value_by_setting_name();
      //---

      // get send cc
      unset($cc);
      $this->model_config->name = "email_whrcpt_nav_no_process_cc";
      $cc_temp = $this->model_config->get_value_by_setting_name();
      $cc = explode("|",$cc_temp);
      //--

      $body = $this->my_phpmailer->email_body_whreceipt_not_process($data);
      $to = $send_to;
      $subject = "Warehouse Receipt not proceed yet..";
