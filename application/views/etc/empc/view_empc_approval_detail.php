...
foreach($v_list_empc_apprv_detail_h as $row){
  ...
  $file_location = "./assets/empcfiles/";
...
?>
  <table class="table table-bordered table-sm">
    ...
    <tr class="table-info">
      ...
      <th>TYPE : <?php echo $row['empc_type_code']." - ".$row['movt_type_name']; ?></th>
    ...
    <tr>
      <th style="width:25%;">
        <div class="input-group">
          ...
          <button class="btn btn-primary btn-sm" style="margin-left:5px;" <?php echo $text_btn_gl; ?> id="btn_empc_gl">Change GL</button>
        </div>
      </th>
  ...
  <input type="hidden" id="empc_code" name="empc_code" value="<?php echo $row['empc_h_code']; ?>">
  <input type="hidden" id="empc_approval_code" name="empc_approval_code" value="<?php echo $row['empc_approval_code']; ?>">
<?php
}
?>
...
<div class="pb-2 mt-1 mb-2" style="font-size:12px;">
  <input type="text" id="remarks" class="form-control" placeholder="put your reject/approval remarks for this empc in here">
  <?php
  if($disable_approve == "disabled"){
    echo "<span style='color:red;'>You only reject this EMC because the DATA is not completed...</span>";
  }
...
<div class="pb-2 mt-1 mb-2 text-right" style="font-size:12px;">
  <button class="btn btn-danger" id="btn_reject_empc" style="margin-right:20px;">Reject</button>
  <button class="btn btn-success" id="btn_approve_empc" <?php echo $disable_approve; ?>>Approve</button>
</div>
...
$('#btn_approve_empc').click(function(){
    var remarks = $('#remarks').val();
    var empc_code = $('#empc_code').val();
    var empc_approval_code = $('#empc_approval_code').val();
    var empc_gl_id_edit = $('#gl_id_apprv_edit').val();
    ...
    if(remarks == '') swal('Error','You should put the remarks','error');
        ...
        closeOnConfirm: false
            ...
            if(result.value){
                ...
                $("#loading_text").text("The EMC is on progress approval, Please wait...");
                $('#loading_body').show();

                $("#btn_approve_empc").attr( "disabled", "disabled" );
                $("#btn_reject_empc").attr( "disabled", "disabled" );

                setTimeout(function () {
                  $.ajax({
                      url       : "<?php echo base_url();?>index.php/empc/empc_approval_process",
                      type      : 'post',
                      dataType  : 'html',
                      data      : {remarks:remarks,empc_code:empc_code,empc_approval_code:empc_approval_code,empc_gl_id_edit:empc_gl_id_edit},
                      success   :  function(respons){
                        ...
                        if(responsdata.status == "1"){
                            ...
                            $('#loading_body').hide();
                            $("#btn_reject_empc").prop( "disabled", false );
                            if(disabled_approve == "") $("#btn_approve_empc").prop( "disabled", false );
                        }
                      }
                  });
                }, 500);
...
//-----------------
$('#btn_reject_empc').click(function(){
  ...
  if(remarks == '') swal('Error','You should put the remarks','error');
      ...
      closeOnConfirm: false
          ...
          if(result.value){
              $("#loading_text").text("The EMC is on progress rejected, Please wait...");
              $('#loading_body').show();
              ...
              setTimeout(function () {
                $.ajax({
                    url       : "<?php echo base_url();?>index.php/empc/empc_rejected",
                    type      : 'post',
                    dataType  : 'html',
                    data      : {remarks:remarks,empc_code:empc_code},
                    success   :  function(respons){
                      if(respons != 0){
                          swal({
                             title: "The EMC has been rejected",
                             type: "success", confirmButtonText: "OK",
                          }).then(function(){
                            setTimeout(function(){
                              location.reload(true);
                          ...
                          Swal('Error!','The EMC was not succesfull rejected','error');
                          $('#loading_body').hide();
                          $("#btn_reject_empc").prop( "disabled", false );
                          if(disabled_approve == "") $("#btn_approve_empc").prop( "disabled", false );
                      }
...
$('#btn_empc_gl').click(function(){
    ...
    $('#modal_gl').load(
        "<?php echo base_url();?>index.php/empc/show_gl_approval",
