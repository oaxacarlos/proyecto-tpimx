...
<div class="container-fluid" style="margin-top:30px;">
  ...
  <div id="empc_report_track_generate"></div>
</div>
...
$('#btn_process').click(function(){
    ...
    if((date_from == '') || (date_to == '')) Swal('Error','You have not filled all the Period','error');
        ...
        $('#empc_report_track_generate').hide();
        $('#progress').show();
        $.ajax({
            url       : "<?php echo base_url();?>index.php/empc/get_empc_report_trackvalue_generate",
            type      : 'post',
            dataType  : 'html',
            data      : {date_from:date_from,date_to:date_to},
            success   :  function(respons){
                $('#progress').hide();
                $('#empc_report_track_generate').show();
                $("#empc_report_track_generate").html(respons);
            }
        });
    }
});
