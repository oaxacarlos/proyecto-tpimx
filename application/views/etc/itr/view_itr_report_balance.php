...
<div class="container-fluid" style="margin-top:30px;">
  ...
  <div id="itr_report_balance_generate"></div>
</div>
...
$('#btn_process').click(function(){
    ...
    if((date_from == '') || (date_to == '')) Swal('Error','You have not filled all the Period','error');
        ...
        $('#itr_report_balance_generate').hide();
        $('#progress').show();
        $.ajax({
            url       : "<?php echo base_url();?>index.php/itr_report/get_itr_report_balance_generate",
            type      : 'post',
            dataType  : 'html',
            data      : {date_from:date_from,date_to:date_to},
            success   :  function(respons){
                $('#progress').hide();
                $('#itr_report_balance_generate').show();
                $("#itr_report_balance_generate").html(respons);
            }
        });
    }
});
