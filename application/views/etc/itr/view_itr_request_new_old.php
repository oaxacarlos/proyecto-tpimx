    ...
    if(plant == '') Swal('Error!','Depot could not blank','error');
      ...
      swal({
        ...
        closeOnConfirm: false
            ...
            if(result.value){
              /*  swal({ title: "Progressing...", text: "Please Wait and dont click anything unti finish",
                  showConfirmButton: false,
                  width: '100%'
                });*/

                $("#loading_text").text("Your ITR is saving to system, Please wait...");
                $('#loading_body').show();
              ...
              $.ajax({
                  ...
                  type      : 'post',
                  dataType  : 'html',
                  data      : {plant:plant,itr_type:itr_type,gl:gl,costcenter:costcenter,remarks:remarks,matid:JSON.stringify(matid),qty:JSON.stringify(qty),uom:JSON.stringify(uom),text1:JSON.stringify(text1),customer:customer,project:project},
                  success   :  function(respons){
                      if(respons != "nosuccess"){
                        //-------------------
                        var file_data = $('#fileToUpload').prop('files')[0];
                        var form_data = new FormData();
                        form_data.append('file', file_data);
                        form_data.append('itr_code', respons);
                        $.ajax({
                        ...
                        //-----------
                        $('#loading_body').hide();
                        //Swal('Success','New ITR '+respons+' has been saved, Wait the Approval from your Department Head','success');
                        //$('#btn_clear_all').click();
                        ...
                        Swal('Error!','New ITR could not created','error');
                    }
                  }
              });
            }
