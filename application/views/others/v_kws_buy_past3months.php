...
<div class="pb-2 mt-1 mb-2 text-right" style="font-size:12px;">
      KWS buy PAST 3 MONTHS
</div>

<div class="container-fluid">
  <div class="row">
    <div class="col-md-2">
      <input type='text' name='datepicker_check' value="<?php echo date('Y-m-d', strtotime($date."-3 month")); ?>" id='datepicker_from' class='required form-control' placeholder='Period From'>
    </div>
    <div class="col-md-2">
...
<div class="container" id="kws_buy_past3months_generate"></div>
...
$('#btn_process').click(function(){
    ...
    if((date_from == '') || (date_to == '')) Swal('Error','You have not filled all the Period','error');
        ...
        $('#kws_buy_past3months_generate').hide();
        $('#progress').show();
        $.ajax({
            url       : "<?php echo base_url();?>index.php/others/get_kws_buy_past3months",
            type      : 'post',
            dataType  : 'html',
            data      : {date_from:date_from,date_to:date_to},
            success   :  function(respons){
                $('#progress').hide();
                $('#kws_buy_past3months_generate').show();
                $("#kws_buy_past3months_generate").html(respons);
            }
        });
    }
});
