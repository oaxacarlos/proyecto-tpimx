...
<div class="containter-fluid">
  <?php if(isset($_SESSION['user_permis']["7"])){ ?>
    <button class="btn btn-success btn-sm" onclick=f_convert_excel_cust_product_cat_filter()>EXCEL</button>
  <?php } ?>

  <?php if(isset($_SESSION['user_permis']["8"])){ ?>
    <button class='btn btn-primary btn-sm' id="copy_button_cust_product_cat_filter" style='margin-left:20px;'>Copy ALL</button>
  <?php } ?>
</div>

<table class="table table-bordered table-striped table-sm" style="margin-top:10px;" id="tbl_cust_product_cat_filter">
  <thead>
    <tr>
      <th rowspan='2'></th>
      ...
      <?php
        foreach($var_cat as $row){
          echo "<th colspan='3' style='text-align:center;'>".$row["cat"]."</th>";
        }
      ?>
    ...
    <tr>
      <?php
        foreach($var_cat as $row){
          echo "<th>Qty</th><th>AMT</th><th>GP</th>";
        }
      ?>
      <th>Qty</th><th>AMT</th><th>GP</th>
    </tr>
  </thead>
  <tbody>
    <?php
