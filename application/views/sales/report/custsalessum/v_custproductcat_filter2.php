...
<div class="containter-fluid">
  <?php if(isset($_SESSION['user_permis']["7"])){ ?>
    <button class="btn btn-success btn-sm" onclick=f_convert_excel_cust_product_cat_filter2()>EXCEL</button>
  <?php } ?>

  <?php if(isset($_SESSION['user_permis']["8"])){ ?>
    <button class='btn btn-primary btn-sm' id="copy_button_cust_product_cat_filter2" style='margin-left:20px;'>Copy ALL</button>
  <?php } ?>
</div>

<table class="table table-bordered table-striped table-sm" style="margin-top:10px;" id="tbl_cust_product_cat_filter2">
  <thead>
    <tr>
      <th rowspan='2'></th>
