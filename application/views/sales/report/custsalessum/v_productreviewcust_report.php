...
$(document).ready(function() {
    $('#tbl_report_productreviewcust').DataTable({
      "paging": false,
      "ordering": false,
    });
});
...
<div class="containter-fluid">
  <?php if(isset($_SESSION['user_permis']["5"])){ ?>
    <button class="btn btn-success btn-sm"  id="btn_export_xlsx_productreviewcust">EXCEL</button>
  <?php } ?>

  <?php if(isset($_SESSION['user_permis']["6"])){ ?>
    <button class='btn btn-primary btn-sm' id="copy_button_productreviewcust" style='margin-left:20px;'>Copy ALL</button>
  <?php } ?>
</div>

<table class="table table-bordered table-striped table-sm" id="tbl_report_productreviewcust" style="margin-top:10px;">
  <?php
    ...
    echo "<thead>";
      ...
      echo "</tr>";
      echo "<tr>";
        echo "<td colspan='2'>Cust Code</td>";
        echo "<td>".$cust_code."</td>";
        echo "<td colspan='14'></td>";
      echo "</tr>";
      echo "<tr>";
        echo "<td colspan='2'>Cust Name</td>";
...
document.getElementById('btn_export_xlsx_productreviewcust').addEventListener('click', function() {
  alert("Your converted to Excel, check your DOWNLOAD folder");
  var cust_code = $("#inp_cust_code").val();
  setTimeout(table2excel3.export(document.querySelector('#tbl_report_productreviewcust'),"ProductReviewCust-"+cust_code),1000);
});
//---
</script>
