  ...
  foreach($var_report as $row){
    foreach($period as $row_period){
      if($row["yearr"]==$row_period["year"] && $row["monthh"]==$row_period["month"]){
          ...
          $data2["amount_outstanding"][$row_period["year"]][$row_period["month"]] = $row["amount_outstanding"];
      }
    }
  }
...
<table class="table table-bordered table-sm table-striped" style='font-size:12px;'>
  <thead>
    ...
    <?php
      foreach($period as $row_period){
          echo "<th class='table-dark' id='title' style='width:120px;'>Closing<br>".$row_period["year"]."-".$row_period["month"] ."</th>";
      }
    ?>
  ...
  <tbody>
    ...
    echo "<td id='number'>".round($var_bo_qty)."</td>";
    echo "</tr>";

    echo "<tr>";
    echo "<td>BackOrder (Amount)</td>";
    foreach($period as $row_period){ echo "<td id='number'>".format_number($data2["amount_outstanding"][$row_period["year"]][$row_period["month"]],1,2)."</td>"; }
    echo "<td id='number'>".format_number($var_report2["amount_outstanding"],1,2)."</td>";
    echo "<td id='number'>".format_number($var_bo_amount,1,2)."</td>";
    echo "</tr>";
