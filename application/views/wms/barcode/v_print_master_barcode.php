...
function print_barcode($file_sn, $serial_number, $file_ext, $item_code, $file_item,$margin_left,$qty){
    $text = "<span class='col-2' style='margin-left:".$margin_left."px;'>";
      //$text.="<span class='col-4'>";
      if(strpos($item_code,"TYP")!== false){
		      if(strlen($item_code) >= 14) $text.= "<div class='col-12'><img src='".base_url().$file_item.$item_code.$file_ext."' width='250'></div>";
		      else $text.= "<div class='col-12'><img src='".base_url().$file_item.$item_code.$file_ext."' width='195'></div>";
	    }
	    else $text.= "<span class='col-11'><img src='".base_url().$file_item.$item_code.$file_ext."' width='160'></span>";

      $text.= "<div class='col' style='margin-top:10px;'><img src='".base_url().$file_sn.$serial_number.$file_ext."' width='250'></div>";
      //$text.="</span>";
...
<body style="width:100%;">
  ...
  <?php
    ...
    foreach($barcode_data as $row){
        if($i==0){
            echo print_barcode($file_folder_sn, $row["sn2"], $file_ext, $row["item_code"],$file_folder_items,-20, $qty_sn2[$row["sn2"]]);
            ...
            if($i%2 == 0){
                ...
                echo print_barcode($file_folder_sn, $row["sn2"], $file_ext,$row["item_code"],$file_folder_items,160, $qty_sn2[$row["sn2"]]);
