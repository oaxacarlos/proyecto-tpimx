...
<div class="row" style="margin-top:20px; margin-left:10px;">
  ...
  <div class="col-1">
    <span class="badge badge-warning">Process InBound</span><br>
...
<div class="row" style="margin-top:20px; margin-left:10px;">
    <table class="table table-bordered table-striped" id="tbl_item" style="width:80%;">
      <thead>
        <tr>
          <th colspan='7'>Search Item<input type="text" id="search_item" class="form-control" onkeypress='return get_item(event)' ></th>
        </tr>
        <tr class="table-info">
          <th>Item</th>
          <th>Desc</th>
          ...
          <th>Value</th>
          <th>
            Master<br>Barcode<br>
            <input type="checkbox" id="check_all" onclick=f_check_all_master_barcode() checked>
          </th>
          <th>Action</th>
        </tr>
      </thead>
...
function f_add_row(item_code, desc, uom){
      ...
      text = text + "<tr id='tbl_item_row_"+glb_index+"'>";
        ...
        text = text + "<td><input type='text' id='tbl_item_src_no_"+glb_index+"' class='form-control'></td>";
        text = text + "<td><input type='text' id='tbl_item_qty_"+glb_index+"' class='form-control' onkeypress='return isNumberKey(event)'></td>";
        text = text + "<td><input type='text' id='tbl_item_value_"+glb_index+"' value='0' class='form-control' onkeypress='return isNumberKey(event)'></td>";
        text = text + "<td><input type='checkbox' id='tbl_master_barcode_"+glb_index+"' checked></td>";
  ...
  swal({
    ...
    closeOnConfirm: false
      ...
      if(result.value){
          ...
          var qty=[]; var master_barcode=[]; var valuee=[];
          ...
          for(i=0;i<glb_index;i++){
              if(check_if_id_exist($("#tbl_item_code_"+i))){
                  ...
                  valuee[ii] = $("#tbl_item_value_"+i).val();

                  if($("#tbl_master_barcode_"+i).is(':checked')) master_barcode[ii] = '1';
                  else master_barcode[ii] = '0';
          ...
          $.ajax({
              url  : "<?php echo base_url();?>index.php/wms/inbound/newinbound/create_new",
              type : "post",
              dataType  : 'html',
              data : {loc:JSON.stringify(loc), item:JSON.stringify(item) , name:JSON.stringify(name), uom:JSON.stringify(uom) , qty:JSON.stringify(qty), src_no:JSON.stringify(src_no), ext_doc:ext_doc, h_loc:h_loc, master_barcode:JSON.stringify(master_barcode), valuee:JSON.stringify(valuee)},
              success: function(data){
                  ...
                  if(responsedata.status == 1){
                        swal({
                          ...
                          setTimeout(function(){
                            $('#loading_body').hide();
                            window.location.href = "<?php echo base_url();?>index.php/wms/inbound/newinbound";
                          },100)
                        });
                  }
