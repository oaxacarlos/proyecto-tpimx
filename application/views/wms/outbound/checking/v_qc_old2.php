        ...
        <?php
          ...
          foreach($var_serial_number as $row){
              ...
              echo "<tr>";
                ...
                echo "<td id='check_sn2_pick_".$i."'>".$row['sn2_pick']."</td>";
...
function f_check_and_assign(data){
    ...
    for(i=0;i<total_row;i++){
        if(data.serial_number == $('#check_serial_number_pick_'+i).text() && data.item_code == $('#check_item_code_'+i).text()){
            check_sn = 1;
            index = i;
            break;
        }
    ...
    if(check_sn == 1){
        ...
        for(i=0;i<total_row;i++){
            if($('#check_item_code_'+i).text() == data.item_code){ item_code_exist = 1; }
            ...
            if( data.location_code == $('#check_location_code_pick_'+i).text() &&
                ...
                $('#check_item_code_'+i).text() == data.item_code
            ){
        ...
        if(check_new == 0){
            if(item_code_exist == 1){
                var temp_bin = data.location_code+"-"+data.zone_code+"-"+data.area_code+"-"+data.rack_code+"-"+data.bin_code;
                show_error(data.item_code + " | "+ data.serial_number +" | "+temp_bin+", El Ubicacion que elegiste es incorrecta");
            }
...
function check_if_already_scanned(data){
    ...
    for(i=0;i<total_row;i++){
        if($('#check_serial_number_scan_'+i).text()!=""){
          if(data.serial_number == $('#check_serial_number_scan_'+i).text()){
            check_sn = 1;
            break;
          }
        }
